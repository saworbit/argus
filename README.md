...
- **Parked slabs are geometry**: A sliding door does not leave the world when it opens - it slides beside the hole it was filling and can come to rest on the very line the graph draws through the doorway. Neither hull carries the slab, so navgen works the parked box out from the entity lump before link building and refuses to mint a walk link through it. A door is open exactly when a bot is walking through it.
