...
- **Parked slabs are geometry**: A door does not leave the world when it opens. A sliding one moves beside the hole it was filling and can come to rest on the very line the graph draws through the doorway; a vertical one that starts open does the reverse and parks in the hole itself. Neither hull carries the slab, so navgen works the parked box out from the entity lump before link building and refuses to mint a walk link through it. A door is open exactly when a bot is walking through it.
- **Movers, whichever class they were built from**: A `func_door` that travels straight up or down and is broad enough to stand on is a platform, not a door - e1m1's `*3` is a 126 by 126 slab whose closed top face is the start area's floor. navgen pads both faces and emits a lift link each way; the runtime boards whenever the slab's own top face is a step from its feet, and asks the door handler for the button when it is not. Press that, then cross here.
...
- **Wall sliding on a blocked step**: A ground step the engine refuses at a wall clips the horizontal velocity into the blocking plane and retries along it, the way stock `SV_FlyMove` does, instead of leaving the ground so the engine's flymove can do the sliding. The bot keeps ground friction and ground acceleration through the contact, which on a ledge is the difference between a slide and a fall, and because `Argus_Accelerate` keeps adding speed while the velocity's projection onto the wish direction is under wishspeed, a bot pressed diagonally into a wall collects the same speed a player does. The probe is a fan across the front of the bounding box, not a single line from the centre: walkmove moves a 32-wide box and a wall that stops it is usually met by a corner, so a centre line finds the wall in 3 per cent of blocked steps where the fan finds it in two thirds. The clipped heading is a new heading, so it goes through the brink guard before it is taken. Throttled marker, `ARGUS <name> wallslide`.
...
- **Typed jump launch discipline, approach speed and air control**: A routed jump hop launches from within 96 units of the seat its link was minted from, turning onto the line to the landing node first, rather than firing at any lip the bot happens to cross at speed. Each link also carries a nav-baked approach speed derived from its unsupported span, landing runway and arc margin. A positive-speed gap walks back onto the extended line and charges through the seat at that target; a zero-speed direct or conservative fallback link keeps the old gate. Measured before the anchor was built: of 53 typed launches in a 120 second dm4 tape, 52 were nowhere near the link's own seat and every one of those flew 45 to 75 degrees off the line to its own landing. Reaper faces the ideal yaw before it simulates a jump for the same reason, and the sprint links have had the anchored, aligned version of this since v3.69; this is that discipline made general. Once airborne on a plain jump or a rocket jump, the wish direction points at the landing node, so air acceleration spends its budget along the arc instead of wherever ordinary steering happens to point, the way Quake 3 hands a weapon jump to `BotAirControl`. A correction that would end against a wall is refused, and control is released on grounding. Markers `ARGUS <name> jumpapproach <target> actual <speed>` and `ARGUS <name> aircontrol`.
...
- **Frame-rate-independent swimming and aiming**: Swim drag and thrust are expressed per second rather than per frame, and the aim spring is sub-stepped so no integration step exceeds 0.02 s. Terminal speed was never the issue for the swim (the drag scales with the impulse, so it settles at 300 u/s at any rate) but the *time constant* was frame-counted, and the aim spring was worse than frame-counted: its damping factor `1 - c * frametime` went negative below about 40 frames a second and was floored, which does not damp the spring, it replaces it with a snap. The lab now runs `+sys_ticrate 0.0139` so it measures the same 71 Hz game a listen server plays; `sys_ticrate` gates Quake's dedicated main loop and the engine default of 0.05 had been running every recorded match at about 19 Hz.
...
- **Motion-proportional aim error**: A moving target contributes a second held error vector, sampled on the same saccade clock and normalized to a fixed 20 Hz observation horizon. It is proportional to observed displacement and inverse aim skill, so a stationary target is bit-for-bit unaffected. Sight loss glides the currently rendered offset to zero without sampling hidden motion, reacquisition starts from zero so one target cannot inherit another's vector, and dedicated and played listen-server rates do not change the error scale.
...
- **Context-aware engagements**: Effective health and armour continuously adjust the safe splash preference and circle tangent without adding radial backpedal pressure at dm4's pit edge; a live Quad compresses the comfort band and invites pressure. At cognition skill 0.5+, a visibly held enemy weapon is remembered as melee, direct fire, splash, or beam and changes the circle, high-ground and ordinary retreat choices. The Rocket Launcher 180u preference floor, 120u/100-stack last-resort gate and Lightning Gun liquid gate remain hard safety boundaries.
- **Secondary-threat positioning**: At cognition skill 0.5+ in deathmatch, the bot remembers the nearest other visible player and uses cover from that player only as a soft tie-breaker at existing strafe, high-ground and retreat decision clocks. It can never veto a missile dodge, make a retreat heading illegal, or churn the cover fan when no two-threat answer exists. `ARGUS <name> crossfire <strafe|backout|retreat>` exposes actual decisions, and death telemetry appends `thirdparty` when another player kills a bot while it is visibly fighting its primary target; lab briefs report `third_party_deaths` directly.
...
- **Retreat by geometry**: Navgen bakes each node's nearest hidden ordinary-walk target and first hop. A low-stack bot tries that graph cover only when the hop is floor-safe and a live trace proves the target hides it from the actual enemy, then falls back to the established heading fan. Cover is a wall between us, never distance. Cornered means fight; the bot shoots the whole way out. `ARGEVT <name> coverroute` marks a baked-cover choice.
...
- **Dynamic item utility scoring**: Scores all live trigger items on the map based on personality appetites, missing health/armour deltas, weapon tiers, and distance attenuation (`score = value * 320 / (dist + 160)`). Distance is the length of the bot-specific graph path the router actually found, plus the endpoint offsets, rather than a straight line through walls. One frame-sliced flood scores every reachable candidate and its winning parent chain becomes the route.
...
- **Temporary bad-place memory**: Two consecutive stalls, non-player deaths, stuck movers and door timeouts create one of four shared avoid spots that expire after 10-25 seconds. Shopping floods, cached paths and active walking routes steer away from them, while airborne, teleporter and mover links only reject a destination inside the mark. Starting inside a mark never traps a bot; moving away remains legal.
...
- **Counterbalanced within-match A/B**: `argus-mcp within-ab dm4 60` puts two control and two candidate bots in the same item economy and repeats with their personality slots swapped. Per-arm bot metrics and cross-arm Bradley-Terry strength reduce match-to-match noise for per-bot QuakeC changes; nav and world changes remain separate-match experiments.
...
6. **Per-Frame Constants Are Tick-Rate Bugs, And So Is The Test Rig**: Physics written as `v = v * k + impulse` per frame behaves differently at different tick rates. Terminal values usually survive (the drag scales with the impulse) but time constants do not, and an integrator can do worse than drift: a damping factor of `1 - c * dt` goes negative on a slow rig, and a floor placed there to stop it ringing silently substitutes a different controller. Express rates per second against `frametime`, sub-step anything stiff, and **measure the rig's rate before calibrating to it, with something better than a one-decimal print**. The "frametime 0.1" recorded here for a year was `ftos` rounding a 0.05 s frame; `sys_ticrate` does gate the dedicated main loop, and the lab had been running every tape at about 19 Hz against the 71 Hz of every played session.
